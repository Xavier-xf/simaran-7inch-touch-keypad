...
static void layout_alarm_list_delete_btn_up(lv_obj_t *obj)
{
    ...
    msg = setting_msgdialog_msg_bg_create(alarm_list_scr_act_obj_id_msg, 460, 200);
    setting_msgdialog_msg_confirm_and_cancel_btn_create(msg, alarm_list_msg_cancel_btn_up, alarm_list_msg_confirm_btn_up);
    setting_msgdialog_msg_create(msg, str_get(LAN_DELETE_FILE));
}
...
static lv_obj_t *layout_alarm_list_list_create(void)
{
    ...
    lv_obj_align_x(obj, NULL, LV_ALIGN_CENTER, 0);
...
static void layout_alarm_list_delete_btn_create(void)
{
    ...
    lv_obj_align(obj, NULL, LV_ALIGN_IN_TOP_RIGHT, -16, 16);
    static rom_bin_info img = rom_bin_info_get(ROM_UI_SECURITY_60_FULL_DELETE_PNG);
...
lv_obj_t *alarm_list_btn_sub_string_create(lv_obj_t *parent, int x, int y, int w, int h, const char *main_string, struct tm *tm)
{
    ...
    if (tm != NULL)
    {
        ...
        lv_obj_set_size(sub_obj, 3000, 35);
        ...
        lv_obj_set_y(sub_obj, y + 32);
        lv_obj_align_x(sub_obj, parent, LV_ALIGN_IN_RIGHT_MID, -50);
    }
...
static void alarm_list_info_create(lv_obj_t *parent)
{
    ...
    for (int i = 0; i < total; i++)
    {
        ...
        alarm_list_btn_sub_string_create(parent, 0, 96 * i, 934, 88,
                                         str_get(ch == 1 ? LAYOUT_SECURITY_LANGUAGE_ID_SECURITY_EMERGENCY_1 : LAYOUT_SECURITY_LANGUAGE_ID_SECURITY_EMERGENCY_2),
                                         &tm);
    }
}
...
{
    printf("enter\n");
    setting_head_label_create(str_get(LAYOUT_SECURITY_LANGUAGE_ID_EMERITY_RECORD));
