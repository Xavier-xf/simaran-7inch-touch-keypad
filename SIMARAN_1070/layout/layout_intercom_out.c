...
#define INTERCOM_OUT_LEFT_ARROW_ID 8
#define INTERCOM_OUT_RIGHT_ARROW_ID 9
#define INTERCOM_OUT_CALL_TO_LABEL_ID 10
#define INTERCOM_OUT_ROOM_NUMBER_LABEL_ID 11
#define INTERCOM_OUT_VOL_SLIDER_BG_ID 12
#define INTERCOM_OUT_VOL_SLIDER_INDICATOR_ID 13
#define INTERCOM_OUT_SOUND_BTN_OBJ_ID 14

static lv_task_t *hung_up_task = NULL;
static lv_task_t *calling_task = NULL;

static int count = 60; // 60秒倒计时

static void intercom_out_hung_up_task_create(void);

static void intercom_out_hung_up_btn_up(lv_obj_t *obj)
{
...
static void intercom_out_countdown_create(lv_obj_t *parent)
{
	...
	lv_obj_set_id(countdown_ring, INTERCOM_OUT_COUNTDOWN_RING_ID);
	...
	lv_obj_set_id(countdown_label, INTERCOM_OUT_COUNTDOWN_LABEL_ID);
	...
	// 立即设置为30S
	lv_label_set_text_fmt(countdown_label, "60S");
...
static void intercom_out_arrows_create(lv_obj_t *parent)
{
	...
	lv_obj_set_id(left_arrow, INTERCOM_OUT_LEFT_ARROW_ID);
	...
	// 左侧提示语
	lv_obj_t *call_to_label = lv_label_create(parent, NULL);
	lv_obj_set_id(call_to_label, INTERCOM_OUT_CALL_TO_LABEL_ID);
	lv_obj_set_size(call_to_label, 181, 39);
	lv_obj_set_pos(call_to_label, 101, 266);
	lv_obj_set_style_local_text_font(call_to_label, LV_LABEL_PART_MAIN, LV_STATE_DEFAULT, FONT_SIZE(32));
	lv_label_set_text(call_to_label, str_get(LAYOUT_INTERCOM_LANG_CALLING_TO_ID));

	// 右侧箭头
	lv_obj_t *right_arrow = lv_obj_create(parent, NULL);
	lv_obj_set_id(right_arrow, INTERCOM_OUT_RIGHT_ARROW_ID);
	...
	lv_obj_set_id(room_label, INTERCOM_OUT_ROOM_NUMBER_LABEL_ID);
	...
	int call_out_number = intercom_number_get();
	if ((call_out_number == GUARD_INTERCOM_NUMBER) || (call_out_number == GUARD_2_INTERCOM_NUMBER))
	{
		lv_label_set_text(room_label, call_out_number == GUARD_INTERCOM_NUMBER ? str_get(LAYOUT_INTERCOM_LANG_GUARD_1_ID) : str_get(LAYOUT_INTERCOM_LANG_GUARD_2_ID));
	}
	else
	{
		lv_label_set_text_fmt(room_label, "%03d", call_out_number);
	}
}
...
static void intercom_out_hung_up_task(lv_task_t *task_t)
{
...
static void intercom_out_hung_up_task_create(void)
{
	lv_obj_t *countdown_label = lv_obj_get_child_form_id(lv_scr_act(), INTERCOM_OUT_COUNTDOWN_LABEL_ID);
	if (countdown_label != NULL)
	{
		lv_obj_set_style_local_text_font(countdown_label, LV_LABEL_PART_MAIN, LV_STATE_DEFAULT, FONT_SIZE(24));
		lv_obj_set_pos(countdown_label, 456, 260);
		if (intercom_state_get() == INTERCOM_STATE_HUNG_UP)
		{
			lv_label_set_text_fmt(countdown_label, "%s", str_get(LAYOUT_INTERCOM_LANG_DISCONNECT_ID));
		}
		else
		{
			lv_label_set_text_fmt(countdown_label, "%s", str_get(LAYOUT_INTERCOM_LANG_NO_ANSWER_ID));
		}
	}

	hung_up_task = lv_layout_task_create(intercom_out_hung_up_task, 2000, LV_TASK_PRIO_LOW, NULL);
}
...
static void intercom_out_key_home(lv_key_t key)
{
...
static void intercom_out_back_btn_up(lv_key_t key)
{
	goto_layout(pLAYOUT(intercom));
}

// 按键绑定表
static const key_binding_t intercom_out_key_bindings[] = {

	KEY_BIND(LV_KEY_HOME, common_btn_key_down, intercom_out_key_home),
	KEY_BIND(LV_KEY_ESC, common_btn_key_down, intercom_out_back_btn_up),
...
{
	// 绑定当前页面的按键
	LAYOUT_KEY_BINDINGS(intercom_out_key_bindings);
	...
	intercom_out_text_icon_create(parent); // 挂断按钮
	intercom_out_countdown_create(parent); // 倒计时环和标签
	intercom_out_arrows_create(parent);	   // 左右箭头和标签

	intercom_state_set(INTERCOM_STATE_CALLING_OUT);
