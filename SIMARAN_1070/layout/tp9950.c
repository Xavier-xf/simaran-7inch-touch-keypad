...
static TP9950_FORMAT door1_format = INVALID_FORMAT;
static TP9950_FORMAT door2_format = INVALID_FORMAT;
static TP9950_FORMAT cctv1_format = INVALID_FORMAT;
static TP9950_FORMAT cctv2_format = INVALID_FORMAT;
static TP9950_FORMAT video_format_check = INVALID_FORMAT;
...
void tp9950_some_reg_get(void)
{
	unsigned char value = 0;
	tp9950_read(0x10, &value);
	printf("===============================>>>>> 亮度 : [%d],\n", value);
	tp9950_read(0x11, &value);
	printf("===============================>>>>> 对比度 : [%d],\n", value);
	tp9950_read(0x12, &value);
	printf("===============================>>>>> 色度 : [%d],\n", value);
	tp9950_read(0x13, &value);
	printf("===============================>>>>> 色调 : [%d],\n", value);
	tp9950_read(0x14, &value);
	printf("===============================>>>>> 锐度 : [%d],\n", value);
}
...
** 返回参数说明：
***/
static TP9950_FORMAT tp9950_format_detection(void)
{
...
void display_U_offset_set(char offset)
{
	I2C_DATA reg;
	reg.reg = 0x0F;
	reg.val = 0;
	if (tp9950_read(reg.reg, &reg.val) == false)
	{
		return;
	}
	printf("======%s======>> U:[%d], V:[%d]\n", __func__, reg.val >> 4 & 0x0F, reg.val & 0x0F);
	reg.val = (reg.val & 0x0F) | (offset << 4 & 0xF0);
	tp9950_write(&reg, 1);
}
void display_V_offset_set(char offset)
{
	I2C_DATA reg;
	reg.reg = 0x0F;
	reg.val = 0;
	...
	printf("======%s======>> U:[%d], V:[%d]\n", __func__, reg.val >> 4 & 0x0F, reg.val & 0x0F);
	reg.val = (reg.val & 0xF0) | (offset & 0x0F);
	tp9950_write(&reg, 1);
}
/***
** 日期: 2022-05-12 14:54
...
void display_bright_adj(int bright, TP9950_FORMAT fmt)
{
	...
	unsigned char base = bright_volume_base_get(st_fmt) - 30;
	unsigned char up = 0, down = 0;
	if ((base >> 6) & 0x01)
	{
		up = 0x3F - (base & 0x3F) + 0x3F;
		down = base & 0x3F;
		// printf("===================>>>>> 负区间,可向下调:[%d], 可向上调:[%d]\n", down, up);
	}
	else
	{
		up = 0x3F - (base & 0x3F);
		down = (base & 0x3F) + 0x3F;
		// printf("===================>>>>> 正区间,可向下调:[%d], 可向上调:[%d]\n", down, up);
	}
	int adj = bright - 4;
	int vol = base + adj * (adj > 0 ? up : down) / 4;
	printf("===============================>>>>> 亮度 : [%d], base : [%d]\n", vol, base);
	I2C_DATA reg;
	reg.reg = 0x10;
	reg.val = vol;
...
void display_color_adj(int color, TP9950_FORMAT fmt)
{
	...
	unsigned char base = color_voluem_base_get(st_fmt) + 20;
	unsigned char up = 0, down = 0;
	if ((base >> 6) & 0x01)
	{
		up = 0x3F - (base & 0x3F);
...
static void display_adjust_write_cfg(TP9950_FORMAT fmt)
{
	...
	{
	case MON_CH_DOOR1:
		bright = user_data_get()->camera.door1.bright;
		cont = user_data_get()->camera.door1.cont;
		color = user_data_get()->camera.door1.color;
		break;
	case MON_CH_DOOR2:
		bright = user_data_get()->camera.door2.bright;
		cont = user_data_get()->camera.door2.cont;
		color = user_data_get()->camera.door2.color;
		break;
	case MON_CH_CCTV1:
		bright = user_data_get()->camera.cctv1.bright;
		cont = user_data_get()->camera.cctv1.cont;
		color = user_data_get()->camera.cctv1.color;
		break;
	case MON_CH_CCTV2:
		bright = user_data_get()->camera.cctv2.bright;
		cont = user_data_get()->camera.cctv2.cont;
		color = user_data_get()->camera.cctv2.color;
		break;
	default:
		return;
	}
...
void tp9950_vin_enable(MON_CH vin_ch, bool en)
{
	I2C_DATA data[2];
	data[0].reg = 0x40;
	data[0].val = 0x00;
...
#define SENSOR_LOAD_FILE_720P25FPS "/etc/isp_tp9950_dvp_960x1080_30fps.conf"
	...
	case TP2802_720P30V2:
		...
		param.main_crop_width = 1280;
		param.main_crop_hight = 720;
	...
	case TP2802_NTSC:
		...
		param.main_crop_width = 938;
		param.main_crop_hight = 240;
		param.main_crop_pos_x = 22;
		param.main_crop_pos_y = 0;
...
{
	int i = 0;
	TP9950_FORMAT format = INVALID_FORMAT;

	tp9950_loss_wait();
	monitor_channel_set(MON_CH_DOOR1);
	monitor_open(false, 0x01);
	tp9950_vin_enable(MON_CH_DOOR1, true);
	...
	tp9950_vin_enable(MON_CH_DOOR1, false);
	...
	monitor_open(false, 0x01);
	tp9950_vin_enable(MON_CH_DOOR2, true);
	// tp9950_vin1_enable(true);
	timeout = 5;
	printf("=============================>> DOOR2:\n\r");
	while (timeout--)
	...
	tp9950_vin_enable(MON_CH_DOOR2, false);
	// tp9950_vin1_enable(false);
	// tp9950_loss_wait();
...
static void *tp9950_task(void *arg)
{
	...
	while (1)
	{
		...
		if (fmt != INVALID_FORMAT)
		{
			if (ch == MON_CH_DOOR1)
			{
				if ((door1_format == INVALID_FORMAT) || (door1_format != video_format_check))
				{
					int timeout = 5, i = 0;
					TP9950_FORMAT mode = INVALID_FORMAT;
					TP9950_FORMAT format = INVALID_FORMAT;
			...
			{
				if ((door2_format == INVALID_FORMAT) || (door2_format != video_format_check))
				{
					int timeout = 5, i = 0;
					TP9950_FORMAT mode = INVALID_FORMAT;
					TP9950_FORMAT format = INVALID_FORMAT;
			...
			{
				if ((cctv1_format == INVALID_FORMAT) || (cctv1_format != video_format_check))
				{
					int timeout = 5, i = 0;
					TP9950_FORMAT mode = INVALID_FORMAT;
					TP9950_FORMAT format = INVALID_FORMAT;
			...
			{
				if ((cctv2_format == INVALID_FORMAT) || (cctv2_format != video_format_check))
				{
					int timeout = 5, i = 0;
					TP9950_FORMAT mode = INVALID_FORMAT;
					TP9950_FORMAT format = INVALID_FORMAT;
		...
		else if (tp9950_format != fmt)
		{
			...
			else
			{
				...
				if (tp9950_detection_valid_cont > 5)
				{
					...
					// N制强制P制
					//  if(ch == MON_CH_DOOR1 && door1_format == INVALID_FORMAT){
					//  	if(fmt == TP2802_NTSC) {
					//  		door1_format = TP2802_PAL;
					//  		fmt = TP2802_PAL;
					//  	}else {
					//  		door1_format = fmt;
					//  	}
					//  }else if(ch == MON_CH_DOOR2 && door2_format == INVALID_FORMAT){
					//  	if(fmt == TP2802_NTSC) {
					//  		door2_format = TP2802_PAL;
					//  		fmt = TP2802_PAL;
					//  	}else {
					//  		door2_format = fmt;
					//  	}
...
bool tp9950_init(void)
{
	...
	pthread_create(&thread_t, NULL, tp9950_task, NULL);
	return true;
}
...
bool video_format_is_invalid(void)
{
	return video_format_check == INVALID_FORMAT ? true : false;
}
