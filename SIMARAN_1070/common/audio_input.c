...
static void audio_input_device_init(void)
{
	...
	ak_ai_set_gain(audio_input_handle_id, 4);   // enable_agc
	...
	ak_ai_enable_nr(audio_input_handle_id, AUDIO_FUNC_DISABLE);
	...
	ak_ai_enable_agc(audio_input_handle_id, AUDIO_FUNC_DISABLE);
...
//	ak_ai_set_aec_attr(audio_input_handle_id, &aec_attr);
	...
	ak_ai_enable_eq(audio_input_handle_id, AUDIO_FUNC_DISABLE);
}
...
bool audio_input_open(unsigned char mask)
{
	...
	if (mask == 0x02)
	{
		ak_ai_enable_aec(audio_input_handle_id, false);
	}
	pthread_mutex_unlock(&audio_input_mutex);
	return true;
}
