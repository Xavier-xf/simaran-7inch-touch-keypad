...
int ak_lcd_parameter_parse_dt1(const void *blob, struct ak_fb *fb_dev)
{
	...
	*/
	...
	int door1_power_pin = fdtdec_get_valoffset_int(blob, node, "door1-pwr-gpio", 1, 0);
	gpio_direction_output(door1_power_pin, 0);
	// printf("======================================>>> get door1 power ping : [%d]\n", door1_power_pin);
	int door2_power_pin = fdtdec_get_valoffset_int(blob, node, "door2-pwr-gpio", 1, 0);
	gpio_direction_output(door2_power_pin, 0);
	// printf("======================================>>> get door2 power ping : [%d]\n", door2_power_pin);
...
static void rgb_pannel_sharepin_cfg(struct ak_fb *fb_dev)
{
	...
	if(fb_dev->panel_info->bus_width == LCD_RGB_BUS_WIDTH_8BIT){
		...
		*/
		...
		/*
		* 24bit mode(RGB888):
		XGPIO_026_FUNC_RGB_D23
        XGPIO_027_FUNC_RGB_D22
...
#ifdef CONFIG_H3_D_CODE
	//wxj
	//暂时先不打开背光，因为logo在后面才会显示，这时屏幕是白色的，整体的效果就是开机闪屏，影响体验
    // /*
