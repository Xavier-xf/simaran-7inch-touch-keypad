...
/*
*  choose firmware for icnt8XXX touch screen IC.
*  37D MIPI Platform:  ts_icn86xx.h
*  37D RGB Platform:   ts_icn85xx.h
*  37E MIPI Platform:  ts_icn85xx_for_mipi.h
*  37E RGB Platform:   ts_icn85xx.h
*/
//#define ICNT86XX
//#define ICNT85XX_FOR_MIPI
#if defined(ICNT86XX)
#include "ts_icn86xx.h"
#elif defined(ICNT85XX_FOR_MIPI)
#include "ts_icn85xx_for_mipi.h"
#else
#include "ts_icn85xx.h"
#endif

#define DRIVER_VERSION "1.0.07"
...
int icn85xx_ts_reset(void)
{
    ...
    gpio_direction_output(icn85xx_ts->board_info->reset_pin, 1);
...
static void icn85xx_report_value_B(void)
{
   ...
   while (nBytes) {
    ...
    for(position = 1; position < POINT_NUM + 1; position++)
    {
        ...
        {
            //pr_info("one touch up: %d\n", position);
            /* button release */
            input_mt_slot(icn85xx_ts->input_dev, position);
        ...
        else if(finger_current[position])
        {
            ...
            input_report_abs(icn85xx_ts->input_dev, ABS_MT_TRACKING_ID, position);
...
static irqreturn_t icn85xx_ts_interrupt(int irq, void *dev_id)
{
    ...
    pr_debug("==========------icn85xx_ts TS Interrupt-----============\n"); 
...
#if CTP_REPORT_PROTOCOL
    pr_info("icn85xx_request_input_dev: Type B================\n");

    input_mt_init_slots(icn85xx_ts->input_dev, POINT_NUM*2, INPUT_MT_DIRECT | INPUT_MT_DROP_UNUSED);
#else
...
static void icn85xx_update(struct icn85xx_ts_data *icn85xx_ts)
{
    ...
    if((icn85xx_ts->ictype == ICN85XX_WITHOUT_FLASH_85) || (icn85xx_ts->ictype == ICN85XX_WITHOUT_FLASH_86))
    {
            ...
            #endif
            #ifdef TS_ICN86XX
            icn85xx_set_fw(sizeof(icnt86xx_fw), &icnt86xx_fw[0]);
            #endif
        #endif
...
static int icn85xx_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	icn85xx_ts->board_info->reset_pin = of_get_named_gpio(np, "reset-gpio", 0);
	if (icn85xx_ts->board_info->reset_pin < 0) {
		err = icn85xx_ts->board_info->reset_pin;
		pr_err("could not get reset-gpio %d\n", err);
		goto err_free_mem;
	}
	err = gpio_request(icn85xx_ts->board_info->reset_pin, "ts-reset-gpio");
	if (err) {
		pr_err("could not request reset-gpio %d\n", err);
		goto err_free_mem;
	}
	icn85xx_ts->board_info->irq_pin = of_get_named_gpio(np, "irq-gpio", 0);
	if (icn85xx_ts->board_info->irq_pin < 0) {
		err = icn85xx_ts->board_info->irq_pin;
		pr_err("could not get irq-gpio %d\n", err);
		goto err_request_irq_gpio;
	}
	err = gpio_request(icn85xx_ts->board_info->irq_pin, "ts-irq-gpio");
	if (err) {
		pr_err("could not request irq-gpio %d\n", err);
		goto err_request_irq_gpio;
	}
...
err_iic_test:
	if (icn85xx_ts->board_info->irq_pin)
		gpio_free(icn85xx_ts->board_info->irq_pin);
err_request_irq_gpio:
	if (icn85xx_ts->board_info->reset_pin)
...
static int __maybe_unused icn85xx_ts_suspend(struct device *dev)
{
    struct icn85xx_ts_data *icn85xx_ts = i2c_get_clientdata(this_client);
...
static int __maybe_unused icn85xx_ts_resume(struct device *dev)
{
    struct icn85xx_ts_data *icn85xx_ts = i2c_get_clientdata(this_client);
...
static const struct of_device_id ak37d_touchscreen_of_ids[] = {
    ...
    { .compatible = "anyka,ak37d-i2c-touchscreen-icn86xx" },
    { .compatible = "anyka,ak37e-i2c-touchscreen-icn85xx" },
    { .compatible = "anyka,ak37e-i2c-touchscreen-icn86xx" },
	{},
};
