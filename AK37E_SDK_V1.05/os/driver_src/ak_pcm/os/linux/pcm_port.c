 ...
 *
 * 备注 本文件面向soundcard层提供依赖于具体操作系统的接口实现
 */
#include "pcm_port.h"

int pcm_clk_enable(struct clk *pclk)
{
	return clk_prepare_enable(pclk);
}

void pcm_clk_disable(struct clk *pclk)
{
	if (!IS_ERR(pclk) && (__clk_is_enabled(pclk)))
		return clk_disable_unprepare(pclk);
	else 
		return ;
}
 ...
 * @return  int
 */
static unsigned long adc_set_samplerate(SND_PARAM *clk_param, unsigned long samplerate)
{
	unsigned long sr;
 ...
 * @return  int
 */
static int adc_set_highspeed(SND_PARAM *clk_param)
{
 ...
 * @return  void
 */
static unsigned long dac_set_samplerate(SND_PARAM *clk_param, unsigned long samplerate)
{
	unsigned long sr;
...
static unsigned long dac_cal_hsclk(unsigned long samplerate)
{
	u32 factor;
	unsigned int i;
...
static int dac_set_highspeed(SND_PARAM *clk_param, unsigned long samplerate)
{
	unsigned long dachs_clk;
...
*/
static unsigned long pdm_set_samplerate(SND_PARAM *clk_param, unsigned long samplerate)
{
	unsigned long pdm_samplerate;

	if (IS_ERR(clk_param->pdm_i2sm))
		return 0;

	if (clk_prepare_enable(clk_param->pdm_i2sm)) {
		pcm_port_err("prepare & enable pdm_i2sm fail p_clk:%p", clk_param->pdm_i2sm);
		return -ENODEV;
	}

	clk_set_rate(clk_param->pdm_i2sm, samplerate);

	pdm_samplerate = clk_get_rate(clk_param->pdm_i2sm);
	pcm_port_func("pdm_samplerate:%lu", pdm_samplerate);

	return pdm_samplerate;
}

static int pdm_set_highspeed(SND_PARAM *clk_param, unsigned long sample_rate)
{
	unsigned long pdm_hsclk;

	if (IS_ERR(clk_param->pdm_hsclk))
		return -EINVAL;

	if (clk_prepare_enable(clk_param->pdm_hsclk)) {
		pcm_port_err("prepare & enable pdm_hsclk fail p_clk:%p", clk_param->pdm_hsclk);
		return -ENODEV;
	}

	clk_set_rate(clk_param->pdm_hsclk, sample_rate);

	pdm_hsclk = clk_get_rate(clk_param->pdm_hsclk);
	pcm_port_func("pdm_hsclk:%lu", pdm_hsclk);
...
{
	unsigned long act_sample_rate;

	act_sample_rate = pdm_set_samplerate(clk_param, sample_rate);
	pdm_set_highspeed(clk_param, sample_rate);

	return act_sample_rate;
}
