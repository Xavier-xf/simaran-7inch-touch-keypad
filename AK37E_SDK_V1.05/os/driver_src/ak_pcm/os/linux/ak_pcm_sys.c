 ...
 * 备注
 * 本文件提供额外的运行时节点来获取当前的各个功能模块的运行时数据
 */
...
static struct akpcm_runtime *get_pcm_runtime_by_type(struct akpcm *pcm, enum akpcm_device type)
{
	struct akpcm_runtime *runtime = NULL;

	list_for_each_entry(runtime, &pcm->runtime_list, list) {
		if (type == runtime->snd_type)
			return runtime;
	}
		...
		struct device_attribute *attr, char *buf)
{
	...
	struct akpcm_runtime *play_rt = get_pcm_runtime_by_type(pcm, AK_PCM_DEV_PLAYBACK);

	if (play_rt == NULL) {
		size = scnprintf(buf, PAGE_SIZE, "No playback");
	} else {
		...
		struct device_attribute *attr, char *buf)
{
	int size;
	struct akpcm *pcm = dev_get_drvdata(dev);
	struct akpcm_runtime *rt = get_pcm_runtime_by_type(pcm, AK_PCM_DEV_PLAYBACK);
		...
		struct device_attribute *attr, char *buf)
{
	int size;
	struct akpcm *pcm = dev_get_drvdata(dev);
	struct akpcm_runtime *loopback_rt = get_pcm_runtime_by_type(pcm, AK_PCM_DEV_LOOPBACK);

	if (loopback_rt == NULL) {
		size = scnprintf(buf, PAGE_SIZE, "No loopback");
		return size;
	}
		...
		struct device_attribute *attr, char *buf)
{
	int size;
	struct akpcm *pcm = dev_get_drvdata(dev);
	struct akpcm_runtime *rt = get_pcm_runtime_by_type(pcm, AK_PCM_DEV_LOOPBACK);
		...
		struct device_attribute *attr, char *buf)
{
	int size;
	struct akpcm *pcm = dev_get_drvdata(dev);
	struct akpcm_runtime *cptr_rt = get_pcm_runtime_by_type(pcm, AK_PCM_DEV_CAPTURE);

	if (cptr_rt == NULL) {
		size = scnprintf(buf, PAGE_SIZE, "No capture");
		return size;
	}
		...
		struct device_attribute *attr, char *buf)
{
	int size;
	struct akpcm *pcm = dev_get_drvdata(dev);
	struct akpcm_runtime *rt = get_pcm_runtime_by_type(pcm, AK_PCM_DEV_CAPTURE);
