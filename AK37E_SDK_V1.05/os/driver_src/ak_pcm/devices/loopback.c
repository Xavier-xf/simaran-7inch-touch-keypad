...
* 需要注意的有
* loopback的生命周期
*     open/read/close/ioctrl等接口 loopback用来存放playback的回送数据
*     涉及到生命周期的管理 可以参考##ANYCLOUD_音频驱动模块设计##中的指定模块的说明
* buffer的管理
*     buffer的hw指针及app指针的行为参考playback
*     每次playback发送完一帧的数据 在其isr函数中就会将数据发送给loopback
*     注意 loopback如果缓冲区满的话 则会停止数据的接收
*     这点与capture不同
* 时间的计算
*     参考get_loopback_timestamp的说明
 ...
 * @rt: pointer to runtime
 *
 * 每次copy_to_loopback都会设置一个时间戳 该时间戳代表着当前这段prd_bytes长的数据最后一个字节数据代表的时间戳
 * 如果是获取prd_bytes长的数据中的某个字节对应的时间戳 需要配置进行时间的重新计算
 */
                    ...
                    loff_t *f_pos, unsigned int f_flags)
{
	...
	do {
		if (wait_event_interruptible(rt->wq,
			...
			ak_pcm_debug("interrupt when wait available_data");
			return -ERESTARTSYS;
		}
