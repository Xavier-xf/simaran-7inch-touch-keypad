...
* 需要注意的有
* capture的生命周期
*     open/read/close/ioctrl等接口 capture 本质上就是负责将采集到的音频数据返回给上层应用
*     这个过程涉及到打开控制器操作句柄 完成对指定的模块 如内部的ADC模块的初始化 包括时钟信号等等
*     然后通过read接口将采集到的数据传递给上层 并在L2的响应中断函数中将继续下一帧的数据的接收
...
*     普遍都是固定的
*     另外需要留意hw/app这两个指针的变化 本质上在capture中 是app指针在追逐hw指针的过程
*     但是假如上层没有及时将数据给取走 则会导致数据堆积在驱动的buffer中 如果剩余的buffer的空间不足
*     的情况下 驱动会抛弃老旧的数据
 ...
 *
 * 更新app指针 boundary的作用参考playback的描述
 */
 ...
 * @rt: pointer to runtime
 *
 * 获取当前buffer的有效数据 buffer中的app跟hw的管理跟playback是一样的
 * 同样线性递增 但本质是个环形buffer 所以获取真实的下标时会进行取余运算
 *
 */
...
static void start_adc(struct akpcm_runtime *rt)
{
	...
	switch (rt->mixer_source) {
		...
		case MIXER_IN_SEL_LINEIN:
			...
			break;
        case MIXER_IN_SEL_MUTE:
            ak_pcm_info("source:MUTE\n");
            break;
		default:
	...
	/*
	* 为避免老数据的影响 启动ADC的时候将现有L2中的数据给清除掉
	*/
 ...
 * L2传输的响应函数
 *     1. 更新hw指针
 *     2. 如果当前的buffer剩余空间不够 则抛弃老旧的数据
 *     3. 启动L2接收下一帧的数据
 *     4. 如果pause的话 则停止接收新的数据
 */
 ...
 *
 * 上层读取数据 这里如果buffer的数据的有效个数不足用户要求的个数的话
 * 会出现阻塞的行为 直到有效数据满足用户的需求数量
 *
 */
...
#ifdef CONFIG_PM
int capture_pm_suspend(void *data)
{
	struct akpcm_runtime *rt = (struct akpcm_runtime *)data;
...
int capture_pm_resume(void *data)
{
	struct akpcm_runtime *rt = (struct akpcm_runtime *)data;
	SND_FORMAT format;
