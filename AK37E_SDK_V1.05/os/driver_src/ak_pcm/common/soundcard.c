 ...
 * ⑦. snd_card_register: register the snd_controller
 * ⑧. snd_card_unregister: unregister the snd_controller
 *
 * 本文件作为硬件抽象层 屏蔽相关输入输出源的细节 保留统一接口给设备层
...
struct snd_card_ops pdm_function_ops =
{
	pdm_open,
	pdm_close,
	codec_pdm_set_format,
	pdm_sethwparam,
};

static struct snd_controller pdm_controller =
{
	.l2_buf_id = BUF_NULL,
	.ops = &pdm_function_ops,
};
#endif
...
struct ak_snd_card g_ak_snd_card =
{
	...
	.controllers[SND_CARD_DAC_LOOPBACK] = NULL,
	.controllers[SND_CARD_I2S0_SEND] = NULL,
	.controllers[SND_CARD_I2S0_RECV] = NULL,
	.controllers[SND_CARD_I2S0_LOOPBACK] = NULL,
	.controllers[SND_CARD_I2S1_RECV] = NULL,
#if defined(AK_AUDIO_PDM_SUPPORT)
	.controllers[SND_CARD_PDM_RECV] = &pdm_controller,
#else
	.controllers[SND_CARD_PDM_RECV] = NULL,
#endif
	.param = NULL,
};
...
{
	if (g_ak_snd_card.controllers[dev]) {
		return 0;
	}

	g_ak_snd_card.controllers[dev] = controller;

	ak_pcm_info("@snd_dev#%d 0x%p", dev, controller);
...
{
	g_ak_snd_card.controllers[dev] = NULL;

	ak_pcm_info("@snd_dev#%d", dev);
...
int snd_card_open(snd_dev dev)
{
	...
	ret = g_ak_snd_card.controllers[dev]->ops->open(dev);
	if (ret)
		return ret;
...
int snd_card_close(snd_dev dev)
{
	...
	return g_ak_snd_card.controllers[dev]->ops->close(dev);
}
...
int snd_card_hwparams(snd_dev dev, enum snd_hwparams_type type, int addr, unsigned int param)
{
	...
	return g_ak_snd_card.controllers[dev]->ops->hwparam(dev, type, addr, param);
}
...
unsigned long snd_card_set_format(snd_dev dev, SND_FORMAT *format)
{
	...
	return g_ak_snd_card.controllers[dev]->ops->set_format(dev, format);
}
